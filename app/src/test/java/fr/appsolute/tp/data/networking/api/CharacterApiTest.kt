...
class CharacterApiTest {
    ...
    fun getCharacterDetailsTest() = runBlocking {
        api.getCharacterById(1).run {
            assertTrue("Request must be a success",this.isSuccessful)
            val data = this.body() ?: throw java.lang.IllegalStateException("Body is null")
            assertEquals("Character must be Rick", firstCharacter ,data)
        }
    }
}